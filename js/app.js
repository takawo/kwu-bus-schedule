...
let specialPeriodsData = null; // 特別ダイヤ期間データ
...
async function loadSpecialPeriods() {
    try {
        const response = await fetch('data/special_periods.json');
        const data = await response.json();
        specialPeriodsData = data.specialPeriods;
    } catch (error) {
        console.error('特別ダイヤ期間データの読み込みに失敗しました:', error);
        // エラー時は空配列を使用
        specialPeriodsData = [];
    }
}
...
function getScheduleType(date) {
    ...
    // specialPeriodsDataが読み込まれていない場合は空配列を使用
    const specialPeriods = specialPeriodsData || [];
...
async function loadSchedule(route = 'motoyama', scheduleType = null) {
    try {
        ...
        if (scheduleType === 'special') {
            fileName = `data/${routePrefix}_special.json`;
        } else if (scheduleType === 'saturday') {
            fileName = `data/${routePrefix}_saturday.json`;
        } else {
            fileName = `data/${routePrefix}.json`;
        }
